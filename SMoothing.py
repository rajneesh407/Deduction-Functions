def createHistoryOneLevelSmoothing(column, data_frame, test_data_frame, primary_id_column, output, values, alpha):
    grp_column = []
    ...
    # calculating history
    alpha = int(alpha)
    globalmean = (data_frame[output].count() - data_frame[output].sum()) / data_frame[output].count()
    # train_new = all_data.copy()
    nrows = data_frame.groupby(column).size()
    means = data_frame.groupby(column)[output].agg('mean')

    score = (np.multiply(means, nrows) + globalmean * alpha) / (nrows + alpha)
    data_frame['smooth'] = data_frame[column]
    data_frame[column + "_history"] = data_frame['smooth'].map(score)
