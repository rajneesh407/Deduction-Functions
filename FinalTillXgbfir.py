...
def dispute_amount_deviation(history, left_history, train, test, customer_identifier, dispute_amount,
    ...
    print("original by average dispute amount created")
    prelim_features.append('original_by_average_dispute_amount')
...
model_dict['primary_key_column']=['pk_deduction_id']
model_dict['output']=['output']
model_dict['dates']=['deduction_created_date','promotion_execution_to_date']
model_dict['entity']=['fk_reason_code_map_id','payer','ar_reason_code','sold_to_party']
...
def xgbmodelling(model_dict,train,test,value):
    ...
    train, test, a = dispute_amount_deviation(train, train, train, test, customer_column,
                                              amount_column, output, 'others', 100, True)

    return train,test

def getKnnList():
    matching = [s for s in prelim_features if "_history" in s]
    return matching
...
def featureEngineering(data,train,test,value):
    ...
    knn1 = KNNfeatures.KNNFeature(train, test)

    train, test, knn_return_list = knn1.createKNNFeature(knn_list, 'original_dispute_amount', 'output',
                                                         'pk_deduction_id', 15)

    base_features=prelim_features+knn_return_list

    return train,test,base_features

train,test,base_features=featureEngineering(data,train,test,90)
print(base_features)