...
def twoLevelHistory(column1, column2, train_Data_frame, test_Data_frame, primary_key_column, output_column, value):
    ...
    train_Data_frame, test_Data_frame = getOthers(column1, train_Data_frame, test_Data_frame, primary_key_column,
                                                  output_column, value)
...
def twoLevelHistorySupport(two_lvel_list, train, test, primary_key_column, output, value):
    ...
    for i in range(length):
        ...
        train, test = twoLevelHistory(two_lvel_list[i][0], two_lvel_list[i][1], train, test, primary_key_column,
                                      'output', '90')
...
train,test=twoLevelHistorySupport(x,train,test,'pk_deduction_id','output',90)
...
print(train['sold_to_party_fk_reason_code_map_id_history'].isnull().sum())
print(test['sold_to_party_fk_reason_code_map_id_history'].isnull().sum())
